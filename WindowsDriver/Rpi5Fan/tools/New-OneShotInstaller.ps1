...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$raw=[IO.File]::ReadAllText($env:RPI5FAN_SELF); $parts=$raw -split '__RPI5FAN_PAYLOAD__\r?\n',2; if($parts.Count -ne 2){throw 'Embedded payload not found.'}; $b64=($parts[1] -replace '\s',''); $zip=Join-Path $env:RPI5FAN_TMP 'payload.zip'; [IO.File]::WriteAllBytes($zip,[Convert]::FromBase64String($b64)); Expand-Archive -LiteralPath $zip -DestinationPath $env:RPI5FAN_TMP -Force; & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $env:RPI5FAN_TMP 'Install-RPi5Fan.ps1'); exit $LASTEXITCODE"
set "RC=%ERRORLEVEL%"
...
'@
    ...
    $text = $stub + ($wrapped -join "`r`n") + "`r`n"
